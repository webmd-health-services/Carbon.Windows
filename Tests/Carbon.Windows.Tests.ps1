...
BeforeAll {
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'Carbon.Windows'|
...
Describe 'Carbon.Windows' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.Windows' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.Windows' -CommandType Function,Cmdlet,Filter))
        {
